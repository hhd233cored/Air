...
function emptyForm(): EditorChatterInput {
  return { slug: "", status: "DRAFT", publishedAt: "", contentMarkdown: "", assets: [] };
}
...
export function ChatterEditor({ onBack }: ChatterEditorProps) {
  ...
  const startNew = () => { setEditingSlug(undefined); setForm(emptyForm()); setSlugTouched(false); if (assetInputRef.current) assetInputRef.current.value = ""; setMessage(""); setError(""); };
  ...
  const selectEntry = async (slug: string) => {
    ...
    try {
      ...
      setForm({ slug: entry.slug, status: entry.status, publishedAt: toLocalDateTime(entry.publishedAt), contentMarkdown: entry.contentMarkdown, assets: [] });
  ...
  const onAssetsChange = (files: FileList | null) => {
    ...
    const slug = (form.slug || slugifyContent(form.contentMarkdown)).trim().toLowerCase();
    if (!form.slug) update("slug", slug);
    const markdown = selected.map((file) => {
      const name = safeAssetName(file.name);
      return `![${name}](/chatter/${slug}/assets/${name})`;
    }).join("\n\n");
    update("contentMarkdown", `${form.contentMarkdown.trimEnd()}${form.contentMarkdown.trim() ? "\n\n" : ""}${markdown}\n`);
    if (assetInputRef.current) assetInputRef.current.value = "";
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      setForm((previous) => ({ ...previous, slug: saved.slug, assets: [], publishedAt: toLocalDateTime(saved.publishedAt) }));
      if (assetInputRef.current) assetInputRef.current.value = "";
  ...
  return (
    <main className="editor-page">
      ...
      <div className="editor-layout">
        ...
        <section className="editor-workspace"><div className="editor-fields glass-card"><label>Slug<input value={form.slug} onChange={(event) => { setSlugTouched(true); update("slug", event.target.value.toLowerCase()); }} placeholder="life-changelog" /></label><div className="editor-field-row editor-field-row--chatter"><label>状态<select value={form.status} onChange={(event) => update("status", event.target.value)}><option value="DRAFT">DRAFT</option><option value="PUBLISHED">PUBLISHED</option><option value="ARCHIVED">ARCHIVED</option></select></label><label>发布时间<input type="datetime-local" value={form.publishedAt} onChange={(event) => update("publishedAt", event.target.value)} /></label></div><div className="editor-file-row editor-file-row--chatter"><label className="editor-file-input">正文图片<input ref={assetInputRef} type="file" accept=".svg,.png,.jpg,.jpeg,.webp,.gif,image/*" multiple onChange={(event) => onAssetsChange(event.target.files)} /></label></div></div><div className="editor-panels"><label className="editor-panel glass-card">Markdown 正文<textarea value={form.contentMarkdown} onChange={(event) => onContentChange(event.target.value)} placeholder="记录此刻…" /></label><section className="editor-panel glass-card"><div className="editor-panel__label">实时预览</div><MarkdownRenderer source={previewSource} /></section></div></section>
      </div>
