...
if (-not $pythonPath) { throw "Python 3.11+ was not found." }

$arguments = @("-m", "app.content_migration")
if ($args -contains "-DryRun" -or $args -contains "--dry-run") { $arguments += "--dry-run" }
Push-Location (Join-Path $projectRoot "backend-python")
try { & $pythonPath @arguments; exit $LASTEXITCODE }
