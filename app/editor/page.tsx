...
export default function EditorPage() {
  ...
  const startNew = () => { setEditingSlug(undefined); setForm(emptyForm()); setSlugTouched(false); setCoverPreview(null); setMessage(""); setError(""); };
  ...
  const selectArticle = async (slug: string) => {
    ...
    try {
      ...
      setSlugTouched(true); setCoverPreview(article.coverUrl); setMessage("");
  ...
  const onCoverChange = (file: File | undefined) => { if (!file) return; if (coverObjectUrl.current) URL.revokeObjectURL(coverObjectUrl.current); coverObjectUrl.current = URL.createObjectURL(file); update("cover", file); setCoverPreview(coverObjectUrl.current); };
  ...
  const save = async () => {
    ...
    try {
      ...
      setEditingSlug(saved.slug); setForm((previous) => ({ ...previous, slug: saved.slug, cover: null, assets: [], publishedAt: toLocalDateTime(saved.publishedAt) })); setCoverPreview(saved.coverUrl);
    ...
    <main className="editor-page">
      ...
      <div className="editor-layout">
        ...
        <section className="editor-workspace">
          <div className="editor-fields glass-card"><label>标题<input value={form.title} onChange={(event) => onTitleChange(event.target.value)} placeholder="文章标题" /></label><label>Slug<input value={form.slug} onChange={(event) => { setSlugTouched(true); update("slug", event.target.value.toLowerCase()); }} placeholder="article-slug" /></label><label>摘要<textarea value={form.summary} onChange={(event) => update("summary", event.target.value)} rows={2} placeholder="文章摘要" /></label><div className="editor-field-row"><label>标签<input value={form.tags} onChange={(event) => update("tags", event.target.value)} placeholder="notes, personal" /></label><label>状态<select value={form.status} onChange={(event) => update("status", event.target.value as EditorArticleInput["status"])}><option value="DRAFT">DRAFT</option><option value="PUBLISHED">PUBLISHED</option><option value="ARCHIVED">ARCHIVED</option></select></label><label>发布时间<input type="datetime-local" value={form.publishedAt} onChange={(event) => update("publishedAt", event.target.value)} /></label></div><div className="editor-file-row"><label className="editor-file-input">封面图（必填）<input type="file" accept=".svg,.png,.jpg,.jpeg,.webp,image/*" onChange={(event) => onCoverChange(event.target.files?.[0])} /></label><label className="editor-file-input">正文图片<input type="file" accept=".svg,.png,.jpg,.jpeg,.webp,.gif,image/*" multiple onChange={(event) => onAssetsChange(event.target.files)} /></label></div>{coverPreview ? <img className="editor-cover-preview" src={coverPreview} alt="封面预览" /> : <p className="editor-muted">请选择一张封面图</p>}</div>
